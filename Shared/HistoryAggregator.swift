...
enum HistoryAggregator {
  static func consumedCalories(from entries: [CalorieEntry], on date: Date) -> Int {
    entries
  ...
  static func groupedConsumedCalories(
    from entries: [CalorieEntry],
    period: HistoryPeriod,
    endingOn date: Date = .now
  ) -> [DailyValue] {
    let calendar = Calendar.current
    ...
    case .daily, .monthly:
      return (0..<period.dayCount).compactMap { offset in
        guard let day = calendar.date(byAdding: .day, value: -offset, to: startOfEndDay) else { return nil }
        return DailyValue(date: day, value: Double(consumedCalories(from: entries, on: day)))
      }
      .reversed()
